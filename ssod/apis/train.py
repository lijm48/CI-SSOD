...
from mmdet.datasets import build_dataset as build_dataset_ori
...
from ssod.runners import build_runner
...
def train_detector(
    ...
    if validate:
        ...
        val_dataset = build_dataset_ori(cfg.data.val, dict(test_mode=True))
        ...
        eval_cfg["by_epoch"] = eval_cfg.get(
            "by_epoch", cfg.runner["type"] != "IterBasedRunner" and cfg.runner["type"] != "IterBasedRunner"
        )
    ...
    if cfg.runner.type == 'MyIterBasedRunner':
        print("my runner!")
        runner.run(data_loaders, cfg.workflow, cfg = cfg, distributed = distributed)
    else:
