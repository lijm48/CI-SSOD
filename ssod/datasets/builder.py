...
from mmdet.datasets.builder import worker_init_fn, _concat_dataset, DATASETS
...
def build_dataset(cfg, default_args=None):
    ...
    from .dataset_wrappers import (ClassBalancedDatasetRFS, ClassBalancedDatasetCrS)
    # import pdb
    # pdb.set_trace()
    ...
    #         build_dataset(cfg['dataset'], default_args), cfg['oversample_thr'])
        ...
        dataset = ClassBalancedDatasetRFS(
            build_dataset(cfg['dataset'], default_args), cfg['oversample_thr'])
    elif cfg['type'] == 'ClassBalancedDatasetCrS':
        dataset = ClassBalancedDatasetCrS(
            build_dataset(cfg['dataset'], default_args), split = cfg['split'])
...
def my_build_sampler(cfg, dist=False, group=False, default_args=None):
    if cfg and ("type" in cfg):
        sampler_type = cfg.get("type")
    else:
        sampler_type = default_args.get("type")
...
def my_build_dataloader(
    ...
    **kwargs,
    ...
    if dist:
        default_sampler_cfg.update(num_replicas=world_size, rank=rank, seed=seed)
        sampler = my_build_sampler(sampler_cfg, dist, shuffle, default_sampler_cfg)

        batch_size = samples_per_gpu
        num_workers = workers_per_gpu
    else:
        sampler = (
            my_build_sampler(sampler_cfg, default_args=default_sampler_cfg)
            if shuffle
            else None
        )
        batch_size = num_gpus * samples_per_gpu
