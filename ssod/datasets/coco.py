...
from .cate import COCO_NOVEL_CATEGORIES,  COCO_NOVEL_CATEGORIES_2, COCO_CATEGORIES 
import gc
import json
...
class SemiCocoDataset(CocoDataset):
    ...
    def __init__(self,
        ...
        if data_split == 1:
            self.novel_ids = [k["id"] for k in COCO_NOVEL_CATEGORIES if k["isthing"] == 1]
        else:
            self.novel_ids = [k["id"] for k in COCO_NOVEL_CATEGORIES_2 if k["isthing"] == 1]
        self.classes_num = [0 for i in range(80)]
        self.annos = []
        self.split = split
        self.count = 0
        ...
        if not test_mode:
            ...
            gc.collect()
        if data_split == 1:
            self._novel_classes = [k["id"] for k in COCO_NOVEL_CATEGORIES if k["isthing"] == 1]
            self._base_classes = [k["id"] for k in COCO_CATEGORIES if k["isthing"] == 1 and k["id"] not in self.novel_ids]
        else:
            self._novel_classes = [k["id"] for k in COCO_NOVEL_CATEGORIES_2 if k["isthing"] == 1]
            self._base_classes = [k["id"] for k in COCO_CATEGORIES if k["isthing"] == 1 and k["id"] not in self.novel_ids]
        # processing pipeline
