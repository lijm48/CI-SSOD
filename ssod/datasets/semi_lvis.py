...
class SemiLVISV05Dataset(CocoDataset):
    ...
    def evaluate(self,
        ...
        if jsonfile_prefix is None:
            ...
            # jsonfile_prefix = osp.join("./results/", 'results')
        else:
            tmp_dir = None
...
SemiLVISDataset = SemiLVISV05Dataset
DATASETS.register_module(name='SemiLVISDataset', module=SemiLVISDataset)


@DATASETS.register_module()
class SemiLVISV1Dataset(SemiLVISDataset):
    ...
    def __init__(self,
        ...
        self.classes_num = [0 for i in range(len(self.CLASSES))]
        self.select_ids = []
        self.unselect_ids = []
        self.annos = []
        self.lambd = 0
        ...
        assert len(self.data_infos) == len(self.annos)
        print("Length of dataset:", len(self.data_infos))
    ...
    def _parse_pseudo_ann_info(self, img_info, pse_annos):
        ...
        for j in range(len(anno)):
            ...
            if 'score' in anno[j]:
                ...
                scores.append(1)
        ann = dict(scores = scores, category_ids = category_ids)
        return ann
    ...
    def _parse_ann_info(self, img_info, ann_info, pseudo_ann_file = None):
    ...
    def load_annotations(self, ann_file, pseudo_ann_file = None):
        ...
        for i in self.img_ids:
            ...
            # train/val split in specified in urls               
            ...
            if self.is_labeled or self.test_mode:
